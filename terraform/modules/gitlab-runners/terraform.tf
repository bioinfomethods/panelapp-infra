resource "aws_ecs_task_definition" "gitlab_runner_tf" {
  ...
  task_role_arn            = "${aws_iam_role.ecs_tasks_gitlab_runner_tf.arn}"
  execution_role_arn       = "${aws_iam_role.ecs_tasks_gitlab_runner_tf.arn}"
  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                      = "${var.app_cpu}"
  memory                   = "${var.app_memory}"
  container_definitions    = "${data.template_file.gitlab_runner.rendered}"
}
...
resource "aws_ecs_service" "gitlab_runner_tf" {
  ...
  network_configuration {
    security_groups = ["${aws_security_group.gitlab_runner_fargate.id}"]
